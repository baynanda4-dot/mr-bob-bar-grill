...
function ReviewCard({ review }) {
  return (
    <blockquote className="card-glass u-lift w-64 shrink-0 px-6 py-8 text-center sm:w-72 md:w-80">
      ...
      <p className="mt-3 text-[10px] uppercase tracking-widest text-white/30">via Tripadvisor</p>
    </blockquote>
  );
}
  ...
  return (
    <section className="overflow-hidden border-t border-white/10 py-20 md:py-24">
      ...
      <div className="px-6">
        ...
        >
          From the first plate off the grill to the last set of the night,
          guests keep coming back for the steaks, the ribs, the wine list,
          and the atmosphere that comes alive every Monday, Wednesday, and
          Saturday.
        </Reveal>
      </div>
      ...
      {mapsHref ? (
        ...
        <p className="mb-8 px-6 text-center text-xs text-white/30">
          Google &amp; Tripadvisor reviews (coming soon)
        </p>
      )}

      <Reveal delay={120} className="flex flex-col justify-center gap-3 px-6 sm:flex-row sm:gap-4">
        <Link href="/reservation" className="btn-primary u-press px-8 py-3 text-center text-sm tracking-widest">
          RESERVE TABLE
        </Link>
        <Link href="/group-reservation" className="btn-outline u-press px-8 py-3 text-center text-sm tracking-widest">
          GROUP RESERVATION
        </Link>
      </Reveal>
    </section>
