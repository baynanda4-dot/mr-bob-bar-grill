...
// `dict` is dictionaries/*/forms.json's `groupReservation` slice, `common`
// is its `common` slice (shared field-chrome strings reused across all 3 forms).
export default function EnquiryForm({ dict, common }) {
  ...
  return (
    <section id="enquiry" className="border-t border-white/10 py-20 px-6 max-w-2xl mx-auto">
      ...
      <p className="text-center text-white/70 mb-10">{dict.subheading}</p>
      <form className="space-y-4" onSubmit={handleSubmit}>
        <fieldset disabled={status === "submitting"} className="m-0 min-w-0 space-y-4 border-0 p-0">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label htmlFor="group-date" className="mb-1 block text-xs text-white/50">{dict.dateLabel}</label>
              <input id="group-date" type="date" min={today} required value={fields.date} onChange={update("date")} className="field p-3 w-full" />
            </div>
            <div>
              <label htmlFor="group-time" className="mb-1 block text-xs text-white/50">{dict.timeLabel}</label>
            ...
            <GuestCountField
              ...
              placeholder={dict.groupSizePlaceholder}
              required
              className="field p-3"
          ...
          <textarea placeholder={dict.messagePlaceholder} aria-label={dict.messagePlaceholder} value={fields.message} onChange={update("message")} className="field p-3 w-full h-24"></textarea>
